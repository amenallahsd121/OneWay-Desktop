 ...
 */
public class OpporuniteService {
...
public List<Opportunite> triUp()  throws SQLException{
         List<Opportunite> users = new ArrayList<>();
        String s = "select * from opportinute ORDER BY date ";
        Statement st = cnx.createStatement();
        ResultSet rs =  st.executeQuery(s);
        while(rs.next()){
            Opportunite p = new Opportunite();
     ...
     public List<Opportunite> triDown()  throws SQLException{
         List<Opportunite> users = new ArrayList<>();
        String s = "select * from opportinute ORDER BY date DESC";
        Statement st = cnx.createStatement();
        ResultSet rs =  st.executeQuery(s);
        while(rs.next()){
     ...
     public List<Opportunite> recuperer(int sizee,String word)  throws SQLException{
         List<Opportunite> users = new ArrayList<>();
        String s = "select * from opportinute";
        Statement st = cnx.createStatement();
        ResultSet rs =  st.executeQuery(s);
           ...
           Boolean find = true;
            ...
            //if(sizee == 1)
            {
             for(int i=0; i<sizee && find == true ; i++)
             {
                ...
                System.out.println(word);
                 if(p.getDepart().charAt(i) == word.charAt(i))
                 {
